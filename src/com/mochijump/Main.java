/*
 * Copyright 2022, Charter Communications, All rights reserved.
 */
...
public class Main {
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException {
        var bipArray = new String[]{
                ...
                "card", "cargo", "carpet", "carry", "cart", "case", "cash", "casino", "castle", "casual", "cat",
                ...
                "key", "kick", "kid", "kidney", "kind", "kingdom", "kiss", "kit", "kitchen", "kite", "kitten", "kiwi",
                "knee", "knife", "knock", "know", "lab", "label", "labor", "ladder", "lady", "lake", "lamp", "language",
                ...
                "page", "pair", "palace", "palm", "panda", "panel", "panic", "panther", "paper", "parade", "parent",
                "park", "parrot", "party", "pass", "patch", "path", "patient", "patrol", "pattern", "pause", "pave",
                ...
                "wine", "wing", "wink", "winner", "winter", "wire", "wisdom", "wise", "wish", "witness", "wolf",
                "woman", "wonder", "wood", "wool", "word", "work", "world", "worry", "worth", "wrap", "wreck",
                "wrestle", "wrist", "write", "wrong", "yard", "year", "yellow", "you", "young", "youth", "zebra",
                "zero", "zone", "zoo"
        };
        var sr = SecureRandom.getInstanceStrong();
        if (args.length > 1 && args[1] != null) {
            sr = SecureRandom.getInstance(args[1]);
        }
        System.out.println("The SecureRandom algorithm being used is " + sr.getAlgorithm());
        var byteSize = 32;
        if (args.length > 0 && args[0] != null) {
            byteSize = Integer.valueOf(args[0]);
            if (byteSize > 1027) {
                System.out.println(
                        "You cannot have a byteSize greater than 1027 as the SHA-256 hash is not long enough to do a "
                                + "proper checksum");
                System.exit(1);
            }
        }
        ...
        sr.nextBytes(bytes);
        System.out.println(bytes.length * 8 + " bits of entropy being used");
        var preCheckSumString = new String();
        for (byte b : bytes) {
            var bString = Integer.toBinaryString(b & 255 | 256).substring(1);
            preCheckSumString = preCheckSumString.concat(bString);
        }
        var digest = MessageDigest.getInstance("SHA-256");
        digest.update(bytes, 0, bytes.length);
        var checksumFullHash = digest.digest();
        var checkSumFullHashString = new String();
        for (byte b : checksumFullHash) {
            var bString = Integer.toBinaryString(b & 255 | 256).substring(1);
            checkSumFullHashString = checkSumFullHashString.concat(bString);
        }
        var size = (preCheckSumString.length());
        var checkSum = checkSumFullHashString.substring(0, (size / 32));
        var fullMnemonicSeed = preCheckSumString.concat(checkSum);
        System.out.println(
                "The complete binary being used for the mnemonic including check sum is: \n" + fullMnemonicSeed);
        var numOfCuts = fullMnemonicSeed.length() / 11;
        var cutUp = new String[numOfCuts];
        for (int i = 0; i < numOfCuts; i++) {
            var startingIndex = i * 11;
            var endingIndex = (i + 1) * 11;
            cutUp[i] = fullMnemonicSeed.substring(startingIndex, endingIndex);
        }
        var mnemonic = new String();
        for (String each : cutUp) {
            int decimal = Integer.parseInt(each, 2);
            mnemonic = mnemonic.concat(" " + bipArray[decimal]);
        }
        System.out.println("Your new "+numOfCuts+" seed phrase is:\n"+mnemonic.substring(1));
    }
}